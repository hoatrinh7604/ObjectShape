...
public class GamePlayController : MonoBehaviour
{
    ...
    {
        if(indexOfRightShape == value)
        {
            //numberContentController.UpdateInfo(currentUserValue, value);
            currentUserValue++;
            if (currentUserValue >= numberOfRightShape)
            {
                UpdateScore();
    ...
    public void NextTurn()
    {
        ...
        for(int i = 0; i < numberOfRightShape; i++)
        {;
            currentArr.Add(indexOfRightShape);
            //currentArrSorted.Add(indexOfRightShape);
        }

        for(int i = 0; i < (leng - numberOfRightShape); i++)
        {
            int index = Random.Range(0, listShape);
            while(index == indexOfRightShape)
            {
                index = Random.Range(0, listShape);
            }
